const enum AndroidSystemURLEnum {
  BLUETOOTH = 'android.settings.BLUETOOTH_SETTINGS',
  WIFI = 'android.settings.WIFI_SETTINGS',
}
const enum IOSSystemURLEnum {
...
export enum WingUniSystemURLEnum {
  BLUETOOTH,
  WIFI,
}
export class WingUniSystem {
   ...
   * @returns boolean
   */
  static openSystemSetting(url: WingUniSystemURLEnum): boolean {
    try {
      if (this.isAndroid) {
        const mainActivity = plus.android.runtimeMainActivity();
        const Intent = plus.android.importClass('android.content.Intent');
        let intentURL = '';
        if (url == WingUniSystemURLEnum.BLUETOOTH) intentURL = AndroidSystemURLEnum.BLUETOOTH;
        if (url == WingUniSystemURLEnum.WIFI) intentURL = AndroidSystemURLEnum.WIFI;

        // @ts-ignore
        mainActivity.startActivity(new Intent(intentURL));
      } else if (this.isIOS) {
        let intentURL = '';
        if (url == WingUniSystemURLEnum.BLUETOOTH) intentURL = IOSSystemURLEnum.BLUETOOTH;
        if (url == WingUniSystemURLEnum.WIFI) intentURL = IOSSystemURLEnum.WIFI;
